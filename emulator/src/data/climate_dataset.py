...
class ClimateDataset(torch.utils.data.Dataset):
    ...
    def get_min_max(self, data):
        if self.channels_last:
            ...
            data = np.moveaxis(
                data, 2, 0
            )  # shape (258, 12, 4, 96, 144) -> (4, 258, 12, 96, 144) easier to calulate statistics
    ...
    def __getitem__(self, index):  # Dict[str, Tensor]):
        ...
        if index >= self.input4mips_ds.length - 1:
            index = (
                index - self.input4mips_ds.length
            )  # just shifiting back by one time the full dataset
    ...
    def __len__(self):
        ...
        ), f"CMIP6 must be num_ensembles times the length of input4mips. Got {self.cmip6_ds.length} and {self.input4mips_ds.length}"
                    ...
                    for y in get_years:
                        for em in self.ensemble_dir:
                            var_dir = os.path.join(
                                em, exp, var, f"{CMIP6_NOM_RES}/{CMIP6_TEMP_RES}/{y}"
                            )
        ...
        if os.path.isfile(
            ...
            for var in variables:
                ...
                for (
                    ...
                    scenarios
                ):  # TODO: implement getting by years! also sub seletction for historical years
